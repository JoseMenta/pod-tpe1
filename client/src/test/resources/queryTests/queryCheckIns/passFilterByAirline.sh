...
./counterClient.sh -DserverAddress=localhost:50051 -Daction=assignCounters -Dsector=C -Dflights='AC987' -Dairline=AirCanada -DcounterCount=3 >/dev/null
...
./passengerClient.sh -DserverAddress=localhost:50051 -Daction=passengerCheckin -Dbooking=XYZ234 -Dsector=A -Dcounter=4 >/dev/null
./passengerClient.sh -DserverAddress=localhost:50051 -Daction=passengerCheckin -Dbooking=DEF345 -Dsector=A -Dcounter=4 >/dev/null
./passengerClient.sh -DserverAddress=localhost:50051 -Daction=passengerCheckin -Dbooking=ABC123 -Dsector=C -Dcounter=1 >/dev/null
./passengerClient.sh -DserverAddress=localhost:50051 -Daction=passengerCheckin -Dbooking=EGH456 -Dsector=C -Dcounter=7 >/dev/null

./counterClient.sh -DserverAddress=localhost:50051 -Daction=checkinCounters -Dsector=C -DcounterFrom=1 -Dairline=AirCanada >/dev/null
./counterClient.sh -DserverAddress=localhost:50051 -Daction=checkinCounters -Dsector=A -DcounterFrom=4 -Dairline=AmericanAirlines >/dev/null
./counterClient.sh -DserverAddress=localhost:50051 -Daction=checkinCounters -Dsector=C -DcounterFrom=7 -Dairline=AmericanAirlines >/dev/null

chmod u+x queryClient.sh

>/tmp/test_results
./queryClient.sh -DserverAddress=localhost:50051 -Daction=checkins -DoutPath=/tmp/test_results -Dairline=AmericanAirlines
